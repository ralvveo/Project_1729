package com.example.project1729._unsorted.bt.basic
...
import com.example.project1729.databinding.ListItemBinding

class ItemAdapter(private val listener: Listener, val adapterType: Boolean) : ListAdapter<ListItem, ItemAdapter.MyHolder>(
    Comparator()
) {
    ...
    class MyHolder(view: View, private val adapter: ItemAdapter, private val listener: Listener, val adapterType: Boolean) : RecyclerView.ViewHolder(view) {
        ...
        init {
            ...
            itemView.setOnClickListener {
                if(adapterType) {
                    try {item1?.device?.createBond()} catch (e: SecurityException) {}
                } else {
                item1?.let { it1 -> listener.onClick(it1) }
    ...
    class Comparator : DiffUtil.ItemCallback<ListItem>(){ //компаратор сравнивает новые и старые значения из cписка и обновляет его, если данные изменились
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyHolder { //эта функция добавляет сами элементы в список
