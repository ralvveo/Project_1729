...
class DopInfoViewModel : ViewModel(), KoinComponent {

    private val checkSaverRepository: CheckSaverRepository by inject()
    fun startCheck(stress: String, stressDuration: String, eyeTraining: String){
        viewModelScope.launch {
            val randomCheck = checkGenerator(stress, stressDuration, eyeTraining)
            checkSaverRepository.saveCheck(randomCheck)
        }
    }


    private fun checkGenerator(stress: String, stressDuration: String, eyeTraining: String): Check {
        ...
        return Check(
            ...
            eyeLoading = stress,
            eyeLoadingDuration = stressDuration,
            eyeTraining = eyeTraining
        )
